...
export function createPostgresCache(args: {
    ...
    const dataTableName = `"public"."data"`; // Assuming 'public' is the shared schema
    ...
    return {
        migrate: async () => {
            ...
            await pool.query(`CREATE TABLE IF NOT EXISTS ${dataTableName}
                (
            ...
            try {
                await client.query(
                    `INSERT INTO ${tableName(
                            "blocks"
                    )} (chainId, blockNumber, blockHash, timestamp)
                     VALUES ($1, $2, $3, $4) ON CONFLICT (chainId, blockhash) DO
                    UPDATE SET
                ...
                for (const event of args.events) {
                    await client.query(
                        `INSERT INTO ${tableName(
                         ...
                         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (chainId, blockNumber, logIndex) DO
                        UPDATE SET
                            name = EXCLUDED.name,
            ...
            try {
                const range = await client.query(
                    `SELECT fromBlock, toBlock
                     ...
                     WHERE chainId = $1
                       AND address = $2
                       AND ((fromBlock <= $3 AND toBlock >= $4) OR (fromBlock <= $5 AND toBlock >= $6)) LIMIT 1`,
                    [
                        args.chainId,
                        args.address,
                        Number(args.toBlock),
        ...
        async insertData(args: { uri: string; data: unknown }): Promise<void> {
            const client = await pool.connect();
            try {
                await client.query(
                    `INSERT INTO ${dataTableName} (uri, data)
                     VALUES ($1, $2) ON CONFLICT (uri) DO NOTHING`,
                    [args.uri, JSON.stringify(args.data)]
                );
            } finally {
                client.release();
            }
        ...
        async getData(args: { uri: string }): Promise<string | null> {
            const client = await pool.connect();
            try {
                const res = await client.query(
                    `SELECT data
                ...
                return res.rows.length > 0 ? JSON.parse(res.rows[0].data) : null;
            } finally {
                client.release();
            }
        }
